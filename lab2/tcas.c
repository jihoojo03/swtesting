...
int
probe (char *eid, int eval)
{
	fprintf(stdout, "%s:%d ", eid, eval) ;
	return eval ;
}
...
int alt_sep_test()
{
    ...
    if (probe("(A&&((B&&C)||D))", (probe("A", enabled) && probe("((B&&C)||D)", (probe("(B&&C)", (probe("B", tcas_equipped) && probe("C", intent_not_known))) || probe("D", !tcas_equipped))))))
    {
	need_upward_RA = Non_Crossing_Biased_Climb() && Own_Below_Threat();
	need_downward_RA = Non_Crossing_Biased_Descend() && Own_Above_Threat();
	if (probe("Y&&Z", (probe("Y", need_upward_RA) && probe("Z", need_downward_RA))))
	    alt_sep = UNRESOLVED; // unreachable
	else if (probe("Y'", need_upward_RA))
	    alt_sep = UPWARD_RA;
	else if (probe("Z'", need_downward_RA))
	    alt_sep = DOWNWARD_RA;
	else
	    alt_sep = UNRESOLVED;
    }
